...
defineProps<{
  modelValue: boolean  //-- モーダルの表示/非表示。true で表示。v-model で双方向バインディング
  title: string  //-- モーダルヘッダーに表示するタイトル（例: 「ルートの種類」「吹き出しの形」）。末尾に「を選択」が付く
  items: HtmlObjectValueItem[]  //-- 選択肢の一覧。html_object_value マスタの値（htmlObjectValueId, valueName, sampleImagePath 等）
}>()
...
const emit = defineEmits<{
  "update:modelValue": [value: boolean]  //-- モーダルを閉じる際に false を発火。v-model の更新に使用
  select: [value: HtmlObjectValueItem]  //-- ユーザーが選択肢をクリックした際に、選択した HtmlObjectValueItem を発火
}>()
...
<template>
  ...
  <Teleport to="body">
    ...
    >
      ...
      <!-- モーダル本体を中央に配置。コンテンツ外クリックで閉じる -->
      <div class="modal-dialog" @click="close">
        <div
          class="modal-content modal-content--narrow card-shadow card-header-full"
          @click.stop
        >
          <!-- ヘッダー（メインカラー背景。「〇〇を選択」） -->
