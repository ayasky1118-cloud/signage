...
const props = withDefaults(
  defineProps<{
    /** 地図の中心座標 [経度, 緯度]。null の場合は東京駅（139.7671, 35.6812）をデフォルト表示 */
    center?: [number, number] | null
    /** 初期ズームレベル（1〜22。15 が市街地の標準的な表示） */
    zoom?: number
    /** MapTiler API キー。未設定または空文字の場合は地図を表示せず「設定してください」メッセージを表示 */
    apiKey?: string | null
    /** true: パン・ズーム・ナビゲーション有効。false: 表示のみ（クリック・ドラッグ無効） */
    interactive?: boolean
  }>(),
...
/**
 * 地図を初期化する。
 * ・mapContainerRef と apiKey が有効な場合のみ実行
 * ・既存の map がある場合は remove してから新規作成（二重初期化防止）
 * ・MapTiler の streets-v2 スタイルを使用
 */
function initMap() {
...
/**
 * 地図の表示領域をコンテナのサイズに合わせてリサイズする。
 * 親要素のサイズが動的に変わった場合（例: モーダル表示、タブ切替）に呼び出す。
 * defineExpose で親コンポーネントから呼び出し可能。
 */
...
/**
 * center または apiKey が変更されたときの処理。
 * ・apiKey が有効で map が存在: center が有効なら setCenter で中心を更新
 * ・apiKey が有効だが map が未作成: initMap で初期化（遅延マウント等のケース）
 */
watch(
