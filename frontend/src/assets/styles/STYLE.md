...
user マスタに存在しない場合の「ログインできません」表示用。

| クラス | 用途 |
|--------|------|
| `.auth-guard-checking` | 突合中（「ログイン確認中...」）の表示 |
| `.auth-guard-denied` | 突合失敗（「ログインできません」）の表示 |
| `.auth-guard-denied-message` | メインメッセージ（青・太字） |
| `.auth-guard-denied-sub` | 補足メッセージ（グレー） |
