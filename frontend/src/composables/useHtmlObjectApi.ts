...
//--
//-- 【用途】
//-- ・OrderDetail（全画面編集）のサイドバーで、ルート描画・テキスト配置・画像配置・吹き出し配置等の選択肢を取得する
...
//-- HTMLオブジェクトマスタ一覧を取得。ルート・テキスト・画像・吹き出し等の全カテゴリとその選択肢を返す。API の camelCase レスポンスを型安全にマッピング。エラー時は throw。
