 ...
 * API キー未設定時は skipped: true で valid: true が返る（検証をスキップ）。
 * 通信エラー・API エラー時は valid: false と message を返す（throw しない）。
 */
...
/**
 * 住所をジオコーディングし、座標（lat, lng）を返す。MapTiler Geocoding を利用。
 * 404: 該当住所なし、503: MAPTILER_API_KEY 未設定。いずれも Error を throw。
 */
