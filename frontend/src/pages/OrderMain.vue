...
function validateRequired(): { valid: boolean; message: string } {
  if (mode.value === "change" && !orderNo.value.trim()) {
    return { valid: false, message: "「注文番号」を選択してください。" }
  }
  if (!orderName.value.trim()) return { valid: false, message: "「注文名」を入力してください。" }
  if (!address.value.trim()) return { valid: false, message: "「住所」を入力してください。" }
  if (!companyName.value.trim()) return { valid: false, message: "「会社名」を選択してください。" }
  if (!templateValue.value.trim()) return { valid: false, message: "「テンプレート」を選択してください。" }
  if (designTypeId.value == null) return { valid: false, message: "「デザイン種別」を選択してください。" }
  ...
  <main id="order-main-page" class="max-w-5xl mx-auto py-12 px-8">
    ...
    <Teleport to="body">
      <div v-show="requiredValidationOpen" class="fixed inset-0 z-50" aria-hidden="false">
        <div class="fixed inset-0 bg-black/40" @click="requiredValidationOpen = false"></div>
        <div class="fixed inset-0 flex items-center justify-center p-4">
          <div class="bg-white rounded-2xl card-shadow border border-slate-200/80 w-full max-w-md overflow-hidden">
            ...
            <div class="px-8 py-5 flex flex-nowrap justify-center">
              <button
                type="button"
                class="px-8 py-2.5 rounded-xl bg-main hover:bg-subBlue text-white text-xs font-bold shadow-md shadow-main/20 transition-all duration-200 whitespace-nowrap"
                @click="requiredValidationOpen = false"
              >
                OK
              </button>
            </div>
